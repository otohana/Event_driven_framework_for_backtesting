...
class plot_performance():
    ...
    def plot_stock_curve(self):
        ...
        for index in range(len(le_x_value)):
            plt.text(le_x_value[index], le_y_value[index]*1.05,
                     "Buy", ha='center', va='bottom', fontsize=8)

        ax2.plot(lexit_x_value, lexit_y_value, 'v', color='red', markersize=8,
                 label='Exit')
        plt.text(lexit_x_value[0], lexit_y_value[0]*1.05,
                 "Sell", ha='center', va='bottom', fontsize=8)
